...
class PARAFAC2(object):
    ...
    def __init__(self, x, rank=20, tol=1e-5, maxiter=500, printitn=100):
        ...
        #self.K = len(self.X) #Mod-OEA
        self.K = len(self.X[0])
        self.L = self.X[0].shape[1]
        #for k in range(1, self.K): #Mod-OEA
        for k in range(self.K):
            #if self.X[k].shape[1] != self.L: #Mod-OEA
            if len(self.X[0][k]) != self.L:
    ...
    def initialize(self):
        ...
        for k in range(self.K):
            print("---------iteration to initialize S and V-------------")
            # print(self.X[0][k].T)
            # print(self.X[0][k].T.shape)
            # print(self.X[0][k].shape)
            # print(self.S)

            self.S[:, :, k] = np.identity(self.rank)
            temp += self.X[0][k].T.dot(self.X[0][k])
            # print(temp)
    ...
    def run(self):
        ...
        for i in range(self.maxiter):
            # update U
            for k in range(self.K):
                [p, sigma, q] = np.linalg.svd(self.H.dot(self.S[:, :, k]).dot(self.V.T).dot(self.X[0][k].T),
                                              full_matrices=False)
